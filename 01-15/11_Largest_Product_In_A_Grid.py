...
def calculate_directions(grid, row, col):
    right, down, dr, dl = 0, 0, 0, 0
    if col <= 19 - 3:
    ...
    if col <= 19 - 3 and row <= 19 - 3:
        dr = grid[row][col] * grid[row + 1][col + 1] * grid[row + 2][col + 2] * grid[row + 3][col + 3] 
    if col >= 3 and row <= 19 - 3:
        dl = grid[row][col] * grid[row + 1][col - 1] * grid[row + 2][col - 2] * grid[row + 3][col - 3] 
    products = [right, down, dr, dl]
    return max(products)
